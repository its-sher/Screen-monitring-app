        ...
        if (respView.status == "success") {
          ...
          if (err == "NO_DATA") {
            ...
            error_g = {
              status: "error",
              message:
                "No Roles, No Permissions!! Please Contact Administrator",
            };
            status_code_g = 403;
          ...
          if (response == true) {
            ...
            if (
              ...
              if (
                    ...
                    if (respView.status == "success") {
                      ...
                      if (err.code == "NO_DATA") {
                        error_g = {
                          status: "error",
                          message:
                            "No Permissions Please contact Administrator",
                        };
                        status_code_g = 204;
                        //for passing to error---------------
                        resolve({
                ...
                roles_data_resp !== undefined &&
                Object.keys(roles_data_resp).length != 0 &&
                roles_data_resp.result == 2
              ) {
                console.log("NO ROLE-STARTS");
                //STEP-4 -- creating access token and refresh token ------------STARTS
                const accesstoken = accesstokencreation(db_user_id);
                //console.log(accesstoken);
                const accesstokenexpiry = moment().add(1, "hours");
                //console.log(accesstokenexpiry);
                ...
                const refreshtokenexpiry = moment().add(1, "years");
                //console.log(refreshtokenexpiry);
                //STEP-4 -- creating access token and refresh token ------------ENDS
                //
                //STEP_5---updateUser----------------STARTS--------------------------------
                let update_payload = {
                  table_name: table_name,
                  query_field: "id",
                  query_value: db_user_id,
